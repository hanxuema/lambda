...
locals {
  routes = {
    ...
    "GET /companies/{id}"           = aws_lambda_function.api_lambda_proxy.arn
...
resource "aws_lambda_permission" "api_gw_invoke_proxy" {
  statement_id  = "AllowExecutionFromAPIGatewayProxy"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.api_lambda_proxy.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.demo_api.execution_arn}/*/*"
}
